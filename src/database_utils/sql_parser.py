...
async def get_sql_tables(db_path: str, sql: str) -> List[str]:
    """
    ...
    """
    db_tables = await get_db_all_tables(db_path)
    try:
        parsed_tables = list(parse_one(sql, read="mysql").find_all(exp.Table))
        correct_tables = [
...
async def get_sql_columns_dict(db_path: str, sql: str) -> Dict[str, List[str]]:
    """
    ...
    for column in sql.find_all(exp.Column):
        ...
        if not table_name:
            ...
            for candidate_table in candidate_tables:
                table_columns = await get_table_all_columns(db_path, candidate_table.name)
...
async def _check_value_exists(
    ...
    result = await execute_sql(db_path, query, "one")
    return result[0] if result else None


async def get_sql_condition_literals(
    db_path: str, sql: str
    ...
    """
    try:
        columns_dict = await get_sql_columns_dict(db_path=db_path, sql=sql)
        used_entities = {}
        for sql_exp in parse_one(sql, read="mysql").flatten():
            for literal in sql_exp.find_all(exp.Literal):
                if literal.parent is None:
                    continue
                    ...
                    for column_exp in literal.parent.find_all(exp.Column):
                        ...
                        for table_name, column_names in columns_dict.items():
                            if column_name.lower() in [
                                col.lower() for col in column_names
                            ]:
                                example_exist = False
                                example = literal.this
                                if "(" in str(literal.parent):
                                    value_check = await _check_value_exists(
                                ...
                                if example_exist:
                                    ...
                                    if (
                                        example
                                        not in used_entities[table_name][column_name]
                                    ):
                                        used_entities[table_name][column_name].append(
                                            example
                                        )
